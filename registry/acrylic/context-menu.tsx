...
//
// SIZE: the kit ships three menus — 13 / 11 / 10 pt (= `default` / `sm` / `xs`).
// Set `size` ONCE on <ContextMenuContent>; it flows through React context to every
// row (and submenu), so the whole menu scales — no need to size each item. `default`
// is the standard system menu (13pt). The Header stays 10pt across sizes (kit-true).
...
// `leading-*` makes the row a deterministic height (kit's 24/22/19px for 13/11/10pt)
// regardless of inherited line-height — otherwise a static mock inside prose would
// drift from the portaled live menu.
...
>(({ className, children, checked, ...props }, ref) => {
  ...
  return (
    <ContextMenuPrimitive.CheckboxItem
      ...
      data-inset="true"
      className={cn(menuItemVariants({ size }), highlightAccent, className)}
      checked={checked}
      {...props}
    >
...
// Section Header — kit's constant 10pt gray label (does not scale with size).
// `inset` aligns it to the row text.
...
// Keyboard-shortcut column — kit's right-aligned tertiary label (⌘K, ⇧⌘N…). Inherits
// the row's font-size (so it scales with the menu) and flips to white when the row is
// highlighted (sitting on the System Blue bar).
function ContextMenuShortcut({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
      data-slot="context-menu-shortcut"
      className={cn(
        "ml-auto pl-4 tracking-wide text-[var(--label-tertiary)] group-data-[highlighted]/mi:text-primary-foreground/80",
        className
      )}
      {...props}
    />
...
export {
  ...
  // For a static mock of the open menu: reuse the exact surface + row geometry
  // (call the cva with the size you're mocking) instead of re-deriving it.
