...
> **订正（上线后）：列表行一个都不要静止填充，包括不可选的文件行。**
> 上面那条"文件行用 `variant="muted"`"是错的，理由有两层：
> ①**填充是可供性**，给"点不动的行"填充等于把语义说反了——最重的视觉落在最惰性的东西上；
> ②三个主题块里 `--acr-card-nested` 与 `--acr-hover` **逐字节相同**（dark 都是
> `rgba(255,255,255,0.05)`），所以"静止的文件行"和"被 hover 的目录行"根本分不出来。
> 现在：所有行静止透明，惰性行改用**变淡**（`opacity-45`，macOS 打开面板里不可选文件的做法），
> 得到四档可区分层级 惰性 < 可交互静止 < hover < 选中（实心 accent）。
> hover 对已选中行不生效——那层白色薄雾会把实心 accent 洗掉。
> 行间补 `gap-0.5`：否则两个相邻填充会把各自 7px 圆角咬成一个带缺口的连体块。
> （`materials.md` 那句「单个凹陷**行**用 `Item variant="muted"`」本身没错，它说的是"要凹陷时"
> 怎么做；这里的结论是列表行**根本不该凹陷**。）
