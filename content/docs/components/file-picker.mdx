...
- **`"dir"`** (default) — the selection is **whatever level you're currently browsing**, not a row
  ...
  no separate "choose this folder without entering it" target, by design (see Decision 4 in the
  design doc): a folder row that both navigates *and* has its own independent "select" hit area is
  the single easiest thing to mis-click in a picker like this.
- **`"file"`** — folders are still shown (as read-only browsing context, `aria-disabled`) and clicking
  one still drills in, but only files can become `value`.
...
all deliberately out of scope — see the design doc's *Explicitly not doing* section if you need one
of these; the intended answer for each is host-side, not a prop this component should grow.
