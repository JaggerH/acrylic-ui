...
It renders **nothing** when idle (so it takes no space); `collapsed` shrinks it to just the cover
plus a progress hairline for an icon-rail sidebar; and `onOpen` (cover/title click) is where you
...
| `collapsed` | `boolean` | `false` | `mini` only — icon-rail mode: cover + progress hairline |
