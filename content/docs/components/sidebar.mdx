---
title: Sidebar
description: The macOS 26 source-list sidebar — frosted vibrancy panel, three densities, neutral-pill selection, collapsible.
---
...
The collapsible preview uses intentionally long workspace, navigation, and
sub-menu labels so truncation can be checked in both the expanded sidebar and
the collapsed hover-card flyout.
...
- **Selected** rows are the neutral pill (`--acr-chip`, a gray fill with a monochrome
  label — no `--primary`/System Blue tint). Unselected rows show **no hover fill**;
  only the selected row is highlighted, matching the macOS source list where hovering
  an item shows nothing (a mouse-down `active:` press is still fed back).
