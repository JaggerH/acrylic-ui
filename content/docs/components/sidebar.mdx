...
the higher-level geometry owner: pass `sidebarWidth` and
`sidebarCollapsedWidth` to Shell so Sidebar's gap reservation, visible surface,
and collapsed icon rail all read the same width tokens.
